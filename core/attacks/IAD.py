...
class IAD(Base):
    ...
    def train(self, schedule=None):
        ...
        wandb.login()
        run = wandb.init(
            project=self.schedule['experiment_name'],
        )
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            msg = time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + "Train CE loss: {:.4f} | Clean Acc: {:.3f} | ASR: {:.3f} | Cross Acc: {:3f}\n".format(
            ...
            if epoch % self.current_schedule['test_epoch_interval'] == 0:
                ...
                log(msg)
                wandb.log({"Clean  Acc": avg_acc_clean, "Poison Acc": avg_acc_bd, "Cross Acc": avg_acc_cross})
