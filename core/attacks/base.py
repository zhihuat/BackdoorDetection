...
from ..utils import Log, select_device
import wandb
...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        wandb.login()
        run = wandb.init(
            project=self.current_schedule['experiment_name'],
        )
        
        ft_compute_sample_grad = self.ft_compute_grad
        ...
        self.log('==========Schedule parameters==========\n')
        self.log(str(self.current_schedule)+'\n')
        ...
        if 'pretrain' in self.current_schedule:
            ...
            self.log(f"Load pretrained parameters: {self.current_schedule['pretrain']}\n")
        
        if 'device' in self.current_schedule and self.current_schedule['device'] == 'GPU':
            self.log('==========Use GPUs to train==========\n')
            GPU_num, device_ids = select_device(self.current_schedule, self.log)
            self.device = device = torch.device(f'cuda:{device_ids[0]}')
        ...
        for i in range(self.current_schedule['epochs']):
            for batch_id, batch in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                predict_digits, _ = self.model(batch_img)
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                self.log(msg)
                wandb.log({"Clean Top1  Acc": top1_correct/total_num, "Cleam Loss": mean_loss})
                ...
                msg = "==========Test result on poisoned test dataset==========\n" + \
                      ...
                      f"Top-1 correct / Total: {top1_correct}/{total_num}, Top-1 accuracy: {top1_correct/total_num}, Top-5 correct / Total: {top5_correct}/{total_num}, Top-5 accuracy: {top5_correct/total_num}, mean loss: {mean_loss}, time: {time.time()-last_time}\n"
                self.log(msg)
                wandb.log({"Posion Top1  Acc": top1_correct/total_num, "Poison Loss": mean_loss})
            ...
            # if (i + 1) % self.current_schedule['sample_grad_interval'] == 0:
            #     ft_per_sample_grads_clean = ft_compute_sample_grad(self.params, self.buffers, self.grad_test_data_clean, self.grad_test_labels_clean).values()
            #     ft_per_sample_grads_poison = ft_compute_sample_grad(self.params, self.buffers, self.grad_test_data_poison, self.grad_test_labels_poison).values()
            #     similarity = []
            #     for grad_clean, grad_poison in zip(ft_per_sample_grads_clean, ft_per_sample_grads_poison):
            #         grad_clean = torch.concat([grad for key, grad in grad_clean])
            #         grad_poison = torch.concat([grad for key, grad in grad_poison])
                    
            #         similarity.append(torch.nn.functional.cosine_similarity(grad_clean, grad_poison))
            #     sim  = np.mean(similarity)
            #     wandb.log({"Similarity": sim})
    ...
    def test(self, schedule=None, model=None, test_dataset=None, poisoned_test_dataset=None, test_loss=None):
        ...
        if test_dataset is not None:
            last_time = time.time()
            # test result on benign test dataset
            predict_digits, labels, mean_loss = self._test(test_dataset, self.device, self.current_schedule['batch_size'], self.current_schedule['num_workers'], model, test_loss)
            total_num = labels.size(0)
            prec1, prec5 = accuracy(predict_digits, labels, topk=(1, 5))
            top1_correct = int(round(prec1.item() / 100.0 * total_num))
            top5_correct = int(round(prec5.item() / 100.0 * total_num))
        ...
        if poisoned_test_dataset is not None:
            last_time = time.time()
            # test result on poisoned test dataset
            predict_digits, labels, mean_loss = self._test(poisoned_test_dataset, self.device, self.current_schedule['batch_size'], self.current_schedule['num_workers'], model, test_loss)
            total_num = labels.size(0)
            prec1, prec5 = accuracy(predict_digits, labels, topk=(1, 5))
            top1_correct = int(round(prec1.item() / 100.0 * total_num))
            top5_correct = int(round(prec5.item() / 100.0 * total_num))
    ...
    def ft_compute_grad(self):
        ...
        self.grad_test_data_clean, self.grad_test_labels_clean = self.test_data[:100]
        self.grad_test_data_poison, self.grad_test_labels_poison = self.poisoned_test_dataset[:100][1]
        return ft_compute_sample_grad
